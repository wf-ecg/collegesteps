...
define('lib/console', [], function () {
    ...
    W.SHIET = {
        ...
        init: function ($) {
            if (!$.fn) throw new Error('init requires jquery');
            this.jqsays = $.browser.msie;
            if (this.trident && !this.jqsays) this.gtie11 = true;
            if (this.trident) $('html').addClass('msie');
            if (this.ltie9) $('html').addClass('ie8');
            return this;
        }
    };
    ...
    (function (c) {
        ...
        if (W.window !== window || !C.debug) {
            ...
            // supplant these
            for (; Boolean(i = C.all.shift());) C[i] = makeMeth(C[i]);
            ...
            for (; Boolean(i = C.all.shift());) makeInfo(i);
